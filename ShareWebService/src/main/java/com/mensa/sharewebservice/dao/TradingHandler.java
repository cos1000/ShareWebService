...
public class TradingHandler implements IHandler<Trading> {
    ...
    public Trading GetById(int id) {
        String sql = "select record from Trading record where record.id = :id";
        List<Trading> answer = handler.GetQuery(sql).setParameter("id", id).setMaxResults(1).getResultList(); 
        if (answer != null && answer.size() > 0) {
            return answer.get(0); 
        } else {
            return null; 
