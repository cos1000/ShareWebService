...
public class ShareInformationHandler  implements IHandler<ShareInformation> {
    ...
    @Override
    public ShareInformation GetById(int id) {
        String sql = "select record from ShareInformation record where record.id = :id";
        List<ShareInformation> answer = handler.GetQuery(sql).setParameter("id", id).setMaxResults(1).getResultList(); 
        if (answer != null && answer.size() > 0) {
            return answer.get(0); 
        } else {
            return null; 
